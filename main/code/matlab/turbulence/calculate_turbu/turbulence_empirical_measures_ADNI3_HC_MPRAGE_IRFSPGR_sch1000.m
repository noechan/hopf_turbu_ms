...
% Load inputs
clear all
cd('/Volumes/ADNI/Projects/2024/ADNI/LONI_IDA/ADNI3/timeseries/inputs_fulldenoising/ADNI3_HC_MPRAGE_IRFSPGR_N238rev/');
...
% Calculates array of distance between the nodes' centroids (Euclidean
% length)
for i=1:NPARCELS
    for j=1:NPARCELS
...
for sub=1:NSUB
    ...
    for lam=1:NLAMBDA
        ...
        for i=1:NPARCELS
            for j=1:NPARCELS
                if i == j
                    continue;  % Exclude self-correlations
                end
                r=rr(i,j);
...
cd('/Volumes/ADNI/Projects/2024/ADNI/LONI_IDA/ADNI3/timeseries/outputs_fulldenoising/Turbulence_ADNI3_HC_MPRAGE_IRFSPGR_N238rev_sch1000/')

save ('turbu_all_measurements_ADNI3_HC_MPRAGE_IRFSPGR_N238rev_sch1000.mat', 'LAMBDA','Turbulence_global_sub', 'Local_KoP_sub','Transfer_sub', 'InformationCascade_sub', 'TransferLambda_sub','gKoP','Meta');
save ('turbu_by_node_ADNI3_HC_MPRAGE_IRFSPGR_N238rev_sch1000.mat', 'Turbulence_node_sub');
